...
fn test_cli_ingest_and_extract() {
    ...
    assert!(output.join("binary.bin").exists(), "binary.bin not extracted");
    assert!(output.join("subdir/nested.txt").exists(), "nested.txt not extracted");
...
fn test_cli_query() {
    ...
    assert!(output_str.contains("Similarity"), "Query output missing similarity");
}
...
fn test_cli_help() {
    ...
    assert!(help_str.contains("ingest"), "Help should mention ingest command");
    assert!(help_str.contains("extract"), "Help should mention extract command");
    assert!(help_str.contains("query"), "Help should mention query command");
}
...
fn test_bit_perfect_reconstruction() {
    ...
    assert_eq!(test_data, extracted_data, "Data not reconstructed bit-perfectly");
...
fn test_large_file_chunking() {
    ...
    assert!(manifest_content.contains("total_chunks"), "Manifest should contain chunk info");
    ...
    assert_eq!(large_data, extracted_data, "Large file not reconstructed correctly");
}
