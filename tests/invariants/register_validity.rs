...
fn bitsliced_vec_strategy(max_nnz: usize, dim: usize) -> impl Strategy<Value = BitslicedTritVec> {
    sparse_vec_strategy(max_nnz, dim)
        .prop_map(move |sparse| BitslicedTritVec::from_sparse(&sparse, dim))
}
...
fn test_word_boundary_trits() {
    for &dim in &[65, 129, 193, 257, 512] {
        ...
        assert!(
            is_valid(&v),
            "Word boundary setting produced invalid vector for dim={}",
            dim
        );
        ...
        for &pos in &boundaries {
            if pos < dim {
                ...
                assert_eq!(
                    v.get(pos),
                    expected,
                    "Trit at boundary {} incorrect for dim={}",
                    pos,
                    dim
                );
            }
        ...
        assert!(
            is_valid(&bound),
            "bind at boundaries invalid for dim={}",
            dim
        );
        ...
        assert!(
            is_valid(&bundled),
            "bundle at boundaries invalid for dim={}",
            dim
        );
    }
}
...
fn test_partial_last_word() {
    ...
    for remainder in 1..64 {
        ...
        assert!(
            is_valid(&v),
            "Setting last trit failed validity for dim={}",
            dim
        );
        ...
        assert!(
            ...
            "Detailed validity check failed for dim={}",
            dim
        );
...
fn test_overlap_detection() {
    ...
    match check_validity_detailed(&v) {
        Err(ValidityError::Overlap {
            word_idx,
            overlap_bits,
        }) => {
            assert_eq!(word_idx, 0, "Overlap should be in word 0");
            assert_eq!(
                overlap_bits & (1u64 << 10),
                1u64 << 10,
                "Overlap should include bit 10"
            );
        }
...
fn test_padding_corruption_detection() {
    ...
    match check_validity_detailed(&v) {
        Err(ValidityError::NonZeroPadding {
            pos_padding, len, ..
        }) => {
            assert_eq!(len, 100);
