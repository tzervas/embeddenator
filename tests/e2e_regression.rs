...
fn test_e2e_comprehensive_workflow() {
    ...
    assert!(ingest_output.status.success(), 
            "Ingest failed: {}", String::from_utf8_lossy(&ingest_output.stderr));
    ...
    assert!(extract_output.status.success(),
            "Extract failed: {}", String::from_utf8_lossy(&extract_output.stderr));
    
    // Step 3: Verify all files
    assert_eq!(fs::read(input.join("small.txt")).unwrap(),
               fs::read(output.join("small.txt")).unwrap(),
               "small.txt mismatch");
    
    assert_eq!(fs::read(input.join("medium.txt")).unwrap(),
               fs::read(output.join("medium.txt")).unwrap(),
               "medium.txt mismatch");
    
    assert_eq!(fs::read(input.join("large.txt")).unwrap(),
               fs::read(output.join("large.txt")).unwrap(),
               "large.txt mismatch");
    
    assert_eq!(fs::read(input.join("tiny.bin")).unwrap(),
               fs::read(output.join("tiny.bin")).unwrap(),
               "tiny.bin mismatch");
    
    assert_eq!(fs::read(input.join("medium.bin")).unwrap(),
               fs::read(output.join("medium.bin")).unwrap(),
               "medium.bin mismatch");
    
    assert_eq!(fs::read(input.join("subdir1/nested1.txt")).unwrap(),
               fs::read(output.join("subdir1/nested1.txt")).unwrap(),
               "nested1.txt mismatch");
    
    assert_eq!(fs::read(input.join("deep/path/to/file/deep.txt")).unwrap(),
...
fn test_e2e_large_dataset_performance() {
    ...
    assert!(ingest_output.status.success());
    assert!(ingest_time.as_secs() < 10, "Ingest took too long: {:?}", ingest_time);
    ...
    assert!(extract_output.status.success());
    assert!(extract_time.as_secs() < 10, "Extract took too long: {:?}", extract_time);
    
    // Verify a few random files
    assert_eq!(fs::read(input.join("file000.txt")).unwrap(),
               fs::read(output.join("file000.txt")).unwrap());
    assert_eq!(fs::read(input.join("file050.txt")).unwrap(),
               fs::read(output.join("file050.txt")).unwrap());
    assert_eq!(fs::read(input.join("file099.txt")).unwrap(),
...
fn test_e2e_query_functionality() {
    ...
    fs::write(input.join("doc1.txt"), "Important document about holographic storage").unwrap();
    fs::write(input.join("doc2.txt"), "Another document with different content").unwrap();
...
fn test_e2e_regression_data_integrity() {
    ...
    assert_eq!(test_data, extracted_text.as_slice(),
...
fn test_e2e_regression_directory_structure() {
    ...
    assert_eq!(fs::read_to_string(output.join("a/file1.txt")).unwrap(), "File 1");
    assert_eq!(fs::read_to_string(output.join("a/b/file2.txt")).unwrap(), "File 2");
    assert_eq!(fs::read_to_string(output.join("a/b/c/file3.txt")).unwrap(), "File 3");
