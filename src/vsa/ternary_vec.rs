...
impl PackedTritVec {
    ...
    /// Cosine similarity: dot(self,other) / sqrt(nnz(self) * nnz(other)).
    pub fn cosine(&self, other: &Self) -> f64 {
        let d = self.dot(other) as f64;
        let nnz_a = self.nnz() as f64;
        let nnz_b = other.nnz() as f64;
        let denom = (nnz_a * nnz_b).sqrt();
        if denom < 1e-12 {
            0.0
        } else {
            d / denom
    ...
    pub fn nnz(&self) -> usize {
        ...
        for &w in &self.data {
            let pos = w & Self::MASK_EVEN_BITS;
            let neg = (w >> 1) & Self::MASK_EVEN_BITS;
            count += (pos | neg).count_ones() as usize;
        }
        count
    }
