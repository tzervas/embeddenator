...
fn bench_document_encoding(c: &mut Criterion) {
    ...
    let query = SparseVec::encode_data(
        b"holographic computing vector symbolic",
        &config,
        Some("/query"),
    );
...
fn bench_render_tasks(c: &mut Criterion) {
    ...
    let render_outputs = [
        ...
        (
            "noise_tile_256",
            generate_noise_pattern(256 * 256 * 4, 0xCAFEBABE),
        ), // RGBA
        (
            "depth_buffer_512",
            generate_noise_pattern(512 * 512 * 4, 0xDEADC0DE),
        ), // float32 depth
    ];
...
fn bench_streaming(c: &mut Criterion) {
    ...
    let encoded_chunks: Vec<_> = chunks
        .iter()
        .enumerate()
        .map(|(i, chunk)| {
    ...
    group.bench_function("rolling_window_bundle", |bencher| {
        bencher.iter(|| {
            let windows: Vec<_> = encoded_chunks
                .windows(window_size)
    ...
    group.bench_function("incremental_similarity", |bencher| {
        bencher.iter(|| {
            ...
            for i in 0..num_chunks - 1 {
                similarities
                    .push(black_box(&encoded_chunks[i]).cosine(black_box(&encoded_chunks[i + 1])));
            }
            black_box(similarities)
        })
    });
