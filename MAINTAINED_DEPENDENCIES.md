...
In-tree workspace members (git subtrees): `crates/embeddenator-vsa`, `crates/embeddenator-io`, `crates/embeddenator-obs`, `crates/embeddenator-retrieval`, `crates/embeddenator-fs` (0.25.0), `crates/embeddenator-interop` (0.22.0). Path deps rewritten to `workspace = true` (fs keeps `embeddenator-io` compression-zstd/lz4 features). default-members stay vsa+io+obs+retrieval (`cargo test` fast). Do not enable fuse / disk-image / python / c-bindings / cuda. `rust-ai` is not imported. Retrieval's crates.io `embeddenator-fs = "0.25"` bench pin stays commented (cycle via benches). `embeddenator-io` and `embeddenator-obs` are true leaves (no `embeddenator-vsa`, `trit-vsa`, or `rust-ai` dependency).
