...
The implementation is located in `src/vsa/simd_cosine.rs` and provides:
...
let a = SparseVec::encode_data(b"document 1", &cfg, None);
let b = SparseVec::encode_data(b"document 2", &cfg, None);
...
```rust
use embeddenator::{ReversibleVSAConfig, SparseVec, simd_cosine};
...
### New Files
- `src/vsa/simd_cosine.rs` - SIMD implementation
- `benches/simd_cosine.rs` - Performance benchmarks
- `tests/simd_cosine_tests.rs` - Comprehensive test suite
- `docs/performance/SIMD_OPTIMIZATION.md` - This documentation

### Modified Files
- `src/lib.rs` - Added `simd_cosine` module
- `src/vsa/vsa.rs` - Updated `cosine()` to support SIMD feature gate, added `cosine_scalar()` public method
