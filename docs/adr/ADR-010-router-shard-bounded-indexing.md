...
- **API**: `EmbrFS::bundle_hierarchically_with_options(prefix, max_chunks_per_node)` in [src/fs/embrfs.rs](../../src/fs/embrfs.rs).
...
- **Implementation**:
  - `EmbrFS::bundle_hierarchically_with_options()` in [src/fs/embrfs.rs](../../src/fs/embrfs.rs).
