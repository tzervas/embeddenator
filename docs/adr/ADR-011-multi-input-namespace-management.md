...
The internal API function `ingest_directory_with_prefix()` in `src/fs/embrfs.rs` handles namespace prefixing:
...
- **Code Location**: 
  ...
  - Namespace prefixing logic: [src/fs/embrfs.rs](../../src/fs/embrfs.rs) (`ingest_directory_with_prefix()`)
...
- **Implementation**:
  ...
  - `src/fs/embrfs.rs`: `ingest_directory_with_prefix()` function
