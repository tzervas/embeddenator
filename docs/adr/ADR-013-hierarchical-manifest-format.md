...
The `docs/architecture/HIERARCHICAL_FORMAT.md` specification document describes the hierarchical manifest structure in detail, but lacks architectural context:
...
- **Manifest Types**: [src/fs/embrfs.rs](../../src/fs/embrfs.rs)
  - `UnifiedManifest` enum
  - `HierarchicalManifest` struct
  - `ManifestLevel`, `ManifestItem`, `SubEngramRef` types
  
- **Flat Format**: [src/fs/embrfs.rs](../../src/fs/embrfs.rs)
  - `FlatManifest` struct (pre-existing)
  
- **Hierarchical Bundling**: [src/fs/embrfs.rs](../../src/fs/embrfs.rs)
...
- **Specification**: [docs/architecture/HIERARCHICAL_FORMAT.md](../architecture/HIERARCHICAL_FORMAT.md)
