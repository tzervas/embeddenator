...
#[cfg(feature = "simd")]
use embeddenator_vsa::simd_cosine::{cosine_scalar, cosine_simd};

#[test]
fn test_cosine_scalar_basic() {
    let a = SparseVec::encode_data(b"hello world", &ReversibleVSAConfig::default(), None);
    let b = SparseVec::encode_data(b"hello world", &ReversibleVSAConfig::default(), None);
    let c = SparseVec::encode_data(b"goodbye world", &ReversibleVSAConfig::default(), None);
...
fn test_cosine_properties() {
    let a = SparseVec::encode_data(b"alpha", &ReversibleVSAConfig::default(), None);
    let b = SparseVec::encode_data(b"beta", &ReversibleVSAConfig::default(), None);
    let c = SparseVec::encode_data(b"gamma", &ReversibleVSAConfig::default(), None);
...
fn test_empty_vectors() {
    ...
    let non_empty = SparseVec::encode_data(b"test", &ReversibleVSAConfig::default(), None);
...
fn test_simd_matches_scalar_basic() {
    ...
    for (data_a, data_b) in test_cases {
        let a = SparseVec::encode_data(data_a, &ReversibleVSAConfig::default(), None);
        let b = SparseVec::encode_data(data_b, &ReversibleVSAConfig::default(), None);
...
// Proptest-based test disabled - proptest is not configured as a feature
/*
#[test]
...
fn test_cosine_with_feature_gate() {
    ...
    let a = SparseVec::encode_data(b"test vector a", &ReversibleVSAConfig::default(), None);
    let b = SparseVec::encode_data(b"test vector b", &ReversibleVSAConfig::default(), None);
...
// This test requires types not currently exported in public API
/*
#[test]
fn test_integration_with_retrieval() {
    // Verify SIMD cosine works correctly in retrieval context
    use embeddenator_vsa::{RerankedResult, TernaryInvertedIndex};
