...
///
/// Newtype around [`trit_vsa::Trit`]. This wrapper does **not** forward
/// `.bind()` — holographic bind is trit **multiplication** (`P*P=P`,
/// self-inverse). `trit_vsa::Trit::bind` is subtraction mod 3 (`P.bind(P)=Z`).
///
/// This is THE foundational type. All math builds on this.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize, Default)]
#[serde(transparent)]
#[repr(transparent)]
pub struct Trit(trit_vsa::Trit);

impl fmt::Debug for Trit {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.0 {
            trit_vsa::Trit::N => write!(f, "N"),
            trit_vsa::Trit::Z => write!(f, "Z"),
            trit_vsa::Trit::P => write!(f, "P"),
        }
    }
}
...
impl fmt::Display for Trit {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.0 {
            trit_vsa::Trit::N => write!(f, "-"),
            trit_vsa::Trit::Z => write!(f, "0"),
            trit_vsa::Trit::P => write!(f, "+"),
        }
    }
}
...
impl Trit {
    /// Negative: -1
    pub const N: Trit = Trit(trit_vsa::Trit::N);
    /// Zero: 0
    pub const Z: Trit = Trit(trit_vsa::Trit::Z);
    /// Positive: +1
    pub const P: Trit = Trit(trit_vsa::Trit::P);

    /// All possible trit values in order
    pub const ALL: [Trit; 3] = [Trit::N, Trit::Z, Trit::P];

    /// Wrap a `trit_vsa::Trit` without exposing `.bind()`.
    #[inline]
    pub(crate) const fn from_inner(inner: trit_vsa::Trit) -> Self {
        Trit(inner)
    }

    /// Borrow the inner storage trit (for packed wrap conversions).
    #[inline]
    pub(crate) const fn inner(self) -> trit_vsa::Trit {
        self.0
    }
    ...
    pub const fn neg(self) -> Trit {
        match self.0 {
            trit_vsa::Trit::N => Trit::P,
            trit_vsa::Trit::Z => Trit::Z,
            trit_vsa::Trit::P => Trit::N,
        }
    }
    ...
    /// Trit multiplication (holographic bind primitive)
    ///
    /// Truth table:
    /// ```text
    ///   × | N  Z  P
    ...
    ///
    /// This is **not** [`trit_vsa::Trit::bind`] (subtraction mod 3).
    #[inline]
    pub const fn mul(self, other: Trit) -> Trit {
        match (self.0, other.0) {
            (trit_vsa::Trit::Z, _) | (_, trit_vsa::Trit::Z) => Trit::Z,
            (trit_vsa::Trit::P, trit_vsa::Trit::P) | (trit_vsa::Trit::N, trit_vsa::Trit::N) => {
                Trit::P
            }
            (trit_vsa::Trit::P, trit_vsa::Trit::N) | (trit_vsa::Trit::N, trit_vsa::Trit::P) => {
                Trit::N
            }
        }
    }
    ...
    ///
    /// Three-argument form is kept locally (`trit_vsa::Trit::add_with_carry` is 2-arg).
    #[inline]
    ...
    pub const fn to_bits(self) -> (bool, bool) {
        match self.0 {
            trit_vsa::Trit::Z => (false, false),
            trit_vsa::Trit::P => (false, true),
            trit_vsa::Trit::N => (true, false),
        }
    }
}
...
mod tests {
    ...
    fn test_tryte3_pack_unpack() {
        for packed in 0..27u8 {
            let tryte = Tryte3::unpack(packed).expect("Tryte3 unpack must succeed for values 0-26");
    ...
    fn test_tryte3_bind_self_inverse() {
        for v in Tryte3::MIN_VALUE..=Tryte3::MAX_VALUE {
            ...
            let tryte =
                Tryte3::from_i8(v).expect("Tryte3::from_i8 must succeed for values in valid range");
    ...
    fn test_word6_pack_unpack() {
        for packed in (0..729u16).step_by(7) {
            ...
            let word = Word6::unpack(packed).expect("Word6 unpack must succeed for values 0-728");
