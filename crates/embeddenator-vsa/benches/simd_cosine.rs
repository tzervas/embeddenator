...
fn bench_cosine_scalar_vs_simd(c: &mut Criterion) {
    ...
    for (name, data_a, data_b) in test_cases {
        ...
        group.bench_with_input(
            ...
            &(&a, &b),
            |bencher, (a, b)| {
                bencher.iter(|| std::hint::black_box(a).cosine_scalar(std::hint::black_box(b)))
            },
        );
        ...
        group.bench_with_input(
            ...
            |bencher, (a, b)| {
                bencher.iter(|| {
                    embeddenator_vsa::simd_cosine::cosine_simd(
...
fn bench_cosine_query_workload(c: &mut Criterion) {
    ...
    group.bench_function("scalar_query_10_docs", |bencher| {
        bencher.iter(|| {
            for doc in &documents {
                std::hint::black_box(query.cosine_scalar(std::hint::black_box(doc)));
            }
        })
    });
    ...
    group.bench_function("simd_query_10_docs", |bencher| {
        bencher.iter(|| {
            for doc in &documents {
                std::hint::black_box(embeddenator_vsa::simd_cosine::cosine_simd(
                    &query,
...
fn bench_cosine_edge_cases(c: &mut Criterion) {
    ...
    group.bench_function("scalar_empty_vs_empty", |bencher| {
        bencher.iter(|| std::hint::black_box(&empty).cosine_scalar(std::hint::black_box(&empty)))
    });

    group.bench_function("scalar_empty_vs_non_empty", |bencher| {
        bencher
            .iter(|| std::hint::black_box(&empty).cosine_scalar(std::hint::black_box(&non_empty)))
    });
    ...
    let config = ReversibleVSAConfig::default();
    let vec = SparseVec::encode_data(b"test data for self-similarity", &config, None);

    group.bench_function("scalar_self_similarity", |bencher| {
        bencher.iter(|| std::hint::black_box(&vec).cosine_scalar(std::hint::black_box(&vec)))
    });
